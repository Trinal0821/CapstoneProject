...
class FuncHostedDeployMgr {
    constructor(workingDir, envName) {
        ...
        this.deploymentInfoFile = path.join(this.deploymentDir, constants_2.FuncHostedBotDeployConfigs.DEPLOYMENT_INFO_FILE);
        this.deploymentZipCacheFile = path.join(this.deploymentDir, constants_2.FuncHostedBotDeployConfigs.DEPLOYMENT_ZIP_CACHE_FILE);
    ...
    async needsToRedeploy() {
        try {
            ...
            const funcIgnoreRules = await this.getIgnoreRules(constants_2.FuncHostedBotDeployConfigs.FUNC_IGNORE_FILE);
            const funcIgnore = await FuncHostedDeployMgr.prepareIgnore(funcIgnoreRules);
            const gitIgnoreRules = await this.getIgnoreRules(constants_2.FuncHostedBotDeployConfigs.GIT_IGNORE_FILE);
    ...
    async zipAFolder(rules) {
        ...
        const normalizeTime = (t) => Math.floor(t / constants_2.CommonConstants.zipTimeMSGranularity);
        ...
        await dir_walk_1.forEachFileAndDir(this.workingDir, (itemPath, stats) => {
            ...
            if (relativePath && !stats.isDirectory() && ig.filter([relativePath]).length > 0) {
                ...
                if (entry) {
                    ...
                    if (mtime >= constants_2.CommonConstants.latestTrustMtime &&
