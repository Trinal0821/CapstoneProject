...
class FunctionsHostedBotImpl extends plugin_1.TeamsBotImpl {
    ...
    async generateArmTemplates(ctx) {
        ...
        const provisionOrchestration = await tools_1.generateBicepFromFile(path.join(bicepTemplateDir, constants_2.Bicep.ProvisionFileName), pluginCtx);
        const provisionModules = await tools_1.generateBicepFromFile(path.join(bicepTemplateDir, constants_1.PathInfo.FuncHostedProvisionModuleTemplateFileName), pluginCtx);
        const configOrchestration = await tools_1.generateBicepFromFile(path.join(bicepTemplateDir, constants_2.Bicep.ConfigFileName), pluginCtx);
        const configModule = await tools_1.generateBicepFromFile(path.join(bicepTemplateDir, constants_1.PathInfo.ConfigurationModuleTemplateFileName), pluginCtx);
        const result = {
            Provision: {
    ...
    async deploy(context) {
        ...
        this.config.provision.subscriptionId = tools_1.getSubscriptionIdFromResourceId(this.config.provision.botWebAppResourceId);
        this.config.provision.resourceGroup = tools_1.getResourceGroupNameFromResourceId(this.config.provision.botWebAppResourceId);
        this.config.provision.siteName = tools_1.getSiteNameFromResourceId(this.config.provision.botWebAppResourceId);
        ...
        const rules = await deployMgr.getIgnoreRules(constants_3.FuncHostedBotDeployConfigs.FUNC_IGNORE_FILE);
        ...
        const config = {
            ...
            timeout: constants_3.CommonConstants.deployTimeoutInMs,
        };
