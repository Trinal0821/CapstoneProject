...
class AadAppClient {
    ...
    static async updateAadAppUsingManifest(stage, manifest, skip = false) {
        var _a;
        try {
            await AadAppClient.retryHanlder(stage, () => aadAppManifestManager_1.AadAppManifestManager.updateAadApp(tokenProvider_1.TokenProvider.token, manifest));
        }
        catch (error) {
            if (skip) {
                ...
                throw AadAppClient.handleError(error, errors_1.UpdateAadAppError, error.message);
            }
        }
    }
    ...
    static async getAadAppUsingManifest(stage, objectId, clientSecret, graphTokenProvider, envName, skip = false) {
        let manifest;
        try {
            ...
            const tenantId = await configs_1.Utils.getCurrentTenantId(graphTokenProvider);
    ...
    static async getAadApp(stage, objectId, clientSecret, graphTokenProvider, envName, skip = false) {
        var _a, _b, _c, _d;
        let getAppObject;
        try {
            if (tokenProvider_1.TokenProvider.audience === tokenProvider_1.TokenAudience.AppStudio) {
