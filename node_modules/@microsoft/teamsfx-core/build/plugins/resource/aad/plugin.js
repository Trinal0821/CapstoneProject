...
class AadAppForTeamsImpl {
    async provision(ctx, isLocalDebug = false) {
        ...
        if (tools_1.isAadManifestEnabled() && tools_1.isConfigUnifyEnabled()) {
        ...
        await tokenProvider_1.TokenProvider.init({ graph: ctx.graphTokenProvider, appStudio: ctx.appStudioToken });
        // Move objectId etc. from input to output.
        const skip = configs_1.Utils.skipAADProvision(ctx, isLocalDebug ? (tools_1.isConfigUnifyEnabled() ? false : true) : false);
        dialog_1.DialogUtils.init(ctx.ui, constants_1.ProgressTitle.Provision, constants_1.ProgressTitle.ProvisionSteps);
        let config = new configs_1.ProvisionConfig(isLocalDebug ? (tools_1.isConfigUnifyEnabled() ? false : true) : false);
        ...
        if (config.objectId) {
            if (!skip) {
                ...
                config = await aadAppClient_1.AadAppClient.getAadApp(telemetryMessage, config.objectId, config.password, ctx.graphTokenProvider, isLocalDebug
                    ? tools_1.isConfigUnifyEnabled()
                        ? ctx.envInfo.envName
                        : undefined
                    : ctx.envInfo.envName);
    ...
    async provisionUsingManifest(ctx, isLocalDebug = false) {
        ...
        config.oauth2PermissionScopeId = (_a = manifest.oauth2Permissions[0]) === null || _a === void 0 ? void 0 : _a.id;
        ...
        if (manifest.id) {
            if (!skip) {
                ...
                config = await aadAppClient_1.AadAppClient.getAadAppUsingManifest(telemetryMessage, manifest.id, config.password, ctx.graphTokenProvider, ctx.envInfo.envName);
    ...
    async postProvision(ctx, isLocalDebug = false) {
        ...
        await tokenProvider_1.TokenProvider.init({ graph: ctx.graphTokenProvider, appStudio: ctx.appStudioToken });
        const config = new configs_1.PostProvisionConfig(isLocalDebug ? (tools_1.isConfigUnifyEnabled() ? false : true) : false);
        config.restoreConfigFromContext(ctx);
        await ((_a = dialog_1.DialogUtils.progress) === null || _a === void 0 ? void 0 : _a.start(constants_1.ProgressDetail.Starting));
        await ((_b = dialog_1.DialogUtils.progress) === null || _b === void 0 ? void 0 : _b.next(constants_1.ProgressDetail.UpdateRedirectUri));
        const redirectUris = AadAppForTeamsImpl.getRedirectUris(config.frontendEndpoint, config.botEndpoint, config.clientId);
    ...
    async checkPermission(ctx, userInfo) {
        ...
        await tokenProvider_1.TokenProvider.init({ graph: ctx.graphTokenProvider, appStudio: ctx.appStudioToken }, tokenProvider_1.TokenAudience.Graph);
    ...
    static getRedirectUris(frontendEndpoint, botEndpoint, clientId) {
        ...
        if (botEndpoint) {
            (_h = (_g = redirectUris.web) === null || _g === void 0 ? void 0 : _g.redirectUris) === null || _h === void 0 ? void 0 : _h.push(`${botEndpoint}/auth-end.html`);
        }
        return redirectUris;
    }
    ...
    async scaffold(ctx) {
        ...
        if (tools_1.isAadManifestEnabled() && tools_1.isConfigUnifyEnabled()) {
            ...
            const appDir = `${ctx.root}/${constants_1.Constants.appPackageFolder}`;
    ...
    isRedirectUrlExist(replyUrls, url, type) {
        return (replyUrls.filter((item) => item.url === url && item.type === type).length >
            0);
    }
    async deploy(ctx) {
        var _a, _b;
    ...
    async loadAndBuildManifest(ctx) {
        var _a;
        const isProvisionSucceeded = !!((_a = ctx.envInfo.state.get("solution")) === null || _a === void 0 ? void 0 : _a.get(solution_1.SOLUTION_PROVISION_SUCCEEDED)) ||
            ctx.answers[constants_1.Constants.DEPLOY_AAD_FROM_CODELENS] === "yes" ||
            ctx.envInfo.envName === "local";
        if (!isProvisionSucceeded) {
            throw results_1.ResultFactory.UserError(errors_1.AadManifestNotProvisioned.name, errors_1.AadManifestNotProvisioned.message());
        }
        const manifest = await aadAppManifestManager_1.AadAppManifestManager.loadAadManifest(ctx);
        if (!manifest.id) {
