...
async function doGenerateBicep(ctx, inputs, addedPlugins, existingPlugins) {
    ...
    const persistRes = await persistBicepTemplates(bicepOrchestrationTemplate, moduleProvisionFiles, moduleConfigFiles, inputs.projectPath);
    if (persistRes.isErr()) {
        return teamsfx_api_1.err(persistRes.error);
    }
    return teamsfx_api_1.ok(undefined); // Nothing to return when success
}
async function persistBicepTemplates(bicepOrchestrationTemplate, moduleProvisionFiles, moduleConfigFiles, projectPath) {
    ...
    if (bicepOrchestrationTemplate.needsGenerateTemplate()) {
        ...
        const templateFolderPath = path_1.default.join(projectPath, templatesFolder);
...
function expandParameterPlaceholdersV3(ctx, envInfo, parameterContent) {
    ...
    const plugins = solutionSettings
        ? solutionSettings.activeResourcePlugins.map((p) => typedi_1.Container.get(p))
        : [];
    const stateVariables = {};
    const availableVariables = { state: stateVariables };
    const envState = envInfo.state;
