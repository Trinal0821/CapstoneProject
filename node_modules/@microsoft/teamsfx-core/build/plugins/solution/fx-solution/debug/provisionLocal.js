    ...
    try {
        ...
        if (inputs.platform === teamsfx_api_1.Platform.VSCode || inputs.platform === teamsfx_api_1.Platform.CLI) {
            ...
            if (includeBot) {
                ...
                const ngrokHttpUrl = await ngrok_1.getNgrokHttpUrl(2544);
                if (!ngrokHttpUrl) {
                    const error = error_1.NgrokTunnelNotConnected();
                    telemetry_1.TelemetryUtils.sendErrorEvent(telemetry_1.TelemetryEventName.setupLocalDebugSettings, error);
                    return teamsfx_api_1.err(error);
...
async function setupLocalEnvironment(ctx, inputs, envInfo) {
    ...
    try {
        ...
        if (inputs.platform === teamsfx_api_1.Platform.VSCode || inputs.platform === teamsfx_api_1.Platform.CLI) {
            ...
            if (includeAAD) {
                if (!envInfo.state[constants_1.ResourcePlugins.SimpleAuth]) {
                    envInfo.state[constants_1.ResourcePlugins.SimpleAuth] = {};
                }
                if (includeSimpleAuth) {
                    envInfo.state[constants_1.ResourcePlugins.SimpleAuth].endpoint = localAuthEndpoint;
                }
            }
            if (includeFrontend) {
                if (!envInfo.state[constants_1.ResourcePlugins.FrontendHosting]) {
                    envInfo.state[constants_1.ResourcePlugins.FrontendHosting] = {};
                }
                envInfo.state[constants_1.ResourcePlugins.FrontendHosting].endpoint = localTabEndpoint;
                envInfo.state[constants_1.ResourcePlugins.FrontendHosting].domain = localTabDomain;
            }
            if (includeBackend) {
                if (!envInfo.state[constants_1.ResourcePlugins.Function]) {
                    envInfo.state[constants_1.ResourcePlugins.Function] = {};
                }
                envInfo.state[constants_1.ResourcePlugins.Function].functionEndpoint = localFuncEndpoint;
            }
            if (includeBot) {
                if (!envInfo.state[constants_1.ResourcePlugins.Bot]) {
                    envInfo.state[constants_1.ResourcePlugins.Bot] = {};
                }
                if (skipNgrok) {
                    ...
                    envInfo.state[constants_1.ResourcePlugins.Bot].siteEndpoint = localBotEndpoint;
                    envInfo.state[constants_1.ResourcePlugins.Bot].validDomain = localBotEndpoint.slice(8);
                }
                else {
                    const ngrokHttpUrl = await ngrok_1.getNgrokHttpUrl(3978);
                    if (!ngrokHttpUrl) {
                        ...
                        envInfo.state[constants_1.ResourcePlugins.Bot].siteEndpoint = ngrokHttpUrl;
                        envInfo.state[constants_1.ResourcePlugins.Bot].validDomain = ngrokHttpUrl.slice(8);
                    }
                }
            }
        }
            ...
            if (includeFrontend) {
                (_c = (_e = envInfo.state)[_f = constants_1.ResourcePlugins.FrontendHosting]) !== null && _c !== void 0 ? _c : (_e[_f] = {});
                envInfo.state[constants_1.ResourcePlugins.FrontendHosting].endpoint = "https://localhost:44302";
                envInfo.state[constants_1.ResourcePlugins.FrontendHosting].domain = "localhost";
            }
            if (includeBot) {
                (_d = (_g = envInfo.state)[_h = constants_1.ResourcePlugins.Bot]) !== null && _d !== void 0 ? _d : (_g[_h] = {});
                const ngrokHttpUrl = await ngrok_1.getNgrokHttpUrl(2544);
                if (!ngrokHttpUrl) {
                    const error = error_1.NgrokTunnelNotConnected();
                    telemetry_1.TelemetryUtils.sendErrorEvent(telemetry_1.TelemetryEventName.setupLocalDebugSettings, error);
...
async function configLocalEnvironment(ctx, inputs, envInfo) {
    ...
    try {
        if (inputs.platform === teamsfx_api_1.Platform.VSCode || inputs.platform === teamsfx_api_1.Platform.CLI) {
            ...
            const botEnvs = includeBot ? await localEnvProvider.loadBotLocalEnvs() : undefined;
            // get config for local debug
            const clientId = (_b = envInfo.state[constants_1.ResourcePlugins.Aad]) === null || _b === void 0 ? void 0 : _b.clientId;
            const clientSecret = (_c = envInfo.state[constants_1.ResourcePlugins.Aad]) === null || _c === void 0 ? void 0 : _c.clientSecret;
            const applicationIdUri = (_d = envInfo.state[constants_1.ResourcePlugins.Aad]) === null || _d === void 0 ? void 0 : _d.applicationIdUris;
            const teamsAppTenantId = (_e = envInfo.state.solution) === null || _e === void 0 ? void 0 : _e.teamsAppTenantId;
            const localTabEndpoint = (_f = envInfo.state[constants_1.ResourcePlugins.FrontendHosting]) === null || _f === void 0 ? void 0 : _f.endpoint;
            const localFuncEndpoint = (_g = envInfo.state[constants_1.ResourcePlugins.Function]) === null || _g === void 0 ? void 0 : _g.functionEndpoint;
            const localAuthEndpoint = (_h = envInfo.state[constants_1.ResourcePlugins.SimpleAuth]) === null || _h === void 0 ? void 0 : _h.endpoint;
            const localAuthPackagePath = (_j = envInfo.state[constants_1.ResourcePlugins.SimpleAuth]) === null || _j === void 0 ? void 0 : _j.simpleAuthFilePath;
            if (includeFrontend) {
                frontendEnvs.teamsfxLocalEnvs[localEnvProvider_1.EnvKeysFrontend.Port] = "53000";
                if (includeAAD) {
                    frontendEnvs.teamsfxLocalEnvs[localEnvProvider_1.EnvKeysFrontend.LoginUrl] = `${localTabEndpoint}/auth-start.html`;
                ...
                try {
                    ...
                    if (envInfo.config.frontend &&
                        ...
                        envInfo.state[constants_1.ResourcePlugins.FrontendHosting].sslCertFile =
                            envInfo.config.frontend.sslCertFile;
                        envInfo.state[constants_1.ResourcePlugins.FrontendHosting].sslKeyFile =
                            envInfo.config.frontend.sslKeyFile;
                        ...
                        envInfo.state[constants_1.ResourcePlugins.FrontendHosting].sslCertFile = localCert.certPath;
                        envInfo.state[constants_1.ResourcePlugins.FrontendHosting].sslKeyFile = localCert.keyPath;
                        frontendEnvs.teamsfxLocalEnvs[localEnvProvider_1.EnvKeysFrontend.SslCrtFile] = localCert.certPath;
                        frontendEnvs.teamsfxLocalEnvs[localEnvProvider_1.EnvKeysFrontend.SslKeyFile] = localCert.keyPath;
                    }
                }
            ...
            if (includeBot) {
                const botId = (_k = envInfo.state[constants_1.ResourcePlugins.Bot]) === null || _k === void 0 ? void 0 : _k.botId;
                const botPassword = (_l = envInfo.state[constants_1.ResourcePlugins.Bot]) === null || _l === void 0 ? void 0 : _l.botPassword;
                botEnvs.teamsfxLocalEnvs[localEnvProvider_1.EnvKeysBot.BotId] = botId;
                botEnvs.teamsfxLocalEnvs[localEnvProvider_1.EnvKeysBot.BotPassword] = botPassword;
                if (includeAAD) {
                    botEnvs.teamsfxLocalEnvs[localEnvProvider_1.EnvKeysBot.ClientId] = clientId;
                    ...
                    botEnvs.teamsfxLocalEnvs[localEnvProvider_1.EnvKeysBot.OauthAuthority] =
                        "https://login.microsoftonline.com";
                    botEnvs.teamsfxLocalEnvs[localEnvProvider_1.EnvKeysBot.LoginEndpoint] = `${(_m = envInfo.state[constants_1.ResourcePlugins.Bot]) === null || _m === void 0 ? void 0 : _m.siteEndpoint}/auth-start.html`;
                    botEnvs.teamsfxLocalEnvs[localEnvProvider_1.EnvKeysBot.ApplicationIdUri] = applicationIdUri;
                }
                if (includeBackend) {
                    backendEnvs.teamsfxLocalEnvs[localEnvProvider_1.EnvKeysBackend.ApiEndpoint] = localFuncEndpoint;
