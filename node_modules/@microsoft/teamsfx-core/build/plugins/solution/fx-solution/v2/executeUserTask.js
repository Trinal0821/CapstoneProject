...
function canAddSso(projectSettings, telemetryReporter) {
    ...
    if (!common_1.isAadManifestEnabled()) {
        const e = new teamsfx_api_1.SystemError(constants_2.SolutionSource, constants_2.SolutionError.NeedEnableFeatureFlag, localizeUtils_1.getLocalizedString("core.addSso.needEnableFeatureFlag"));
    ...
    if (!(solutionSettings.hostType === question_2.HostTypeOptionAzure.id)) {
        const e = new teamsfx_api_1.UserError(constants_2.SolutionSource, constants_2.SolutionError.AddSsoNotSupported, localizeUtils_1.getLocalizedString("core.addSso.onlySupportAzure"));
        return teamsfx_api_1.err(util_1.sendErrorTelemetryThenReturnError(constants_2.SolutionTelemetryEvent.AddSso, e, telemetryReporter));
    }
    ...
    if (solutionSettings.capabilities.includes(question_2.BotOptionItem.id)) {
        ...
        if (botHostType === common_1.BotHostTypes.AzureFunctions) {
            const e = new teamsfx_api_1.UserError(constants_2.SolutionSource, constants_2.SolutionError.AddSsoNotSupported, localizeUtils_1.getLocalizedString("core.addSso.functionNotSupport"));
            return teamsfx_api_1.err(util_1.sendErrorTelemetryThenReturnError(constants_2.SolutionTelemetryEvent.AddSso, e, telemetryReporter));
        }
    }
    // Check whether SSO is enabled
    const activeResourcePlugins = solutionSettings.activeResourcePlugins;
    const containTabSsoItem = solutionSettings.capabilities.includes(question_2.TabSsoItem.id);
    const containTab = solutionSettings.capabilities.includes(question_2.TabOptionItem.id);
    const containBotSsoItem = solutionSettings.capabilities.includes(question_2.BotSsoItem.id);
    const containBot = solutionSettings.capabilities.includes(question_2.BotOptionItem.id);
    ...
    if (((containTabSsoItem && !containBot) ||
        ...
        const e = new teamsfx_api_1.UserError(constants_2.SolutionSource, constants_2.SolutionError.SsoEnabled, localizeUtils_1.getLocalizedString("core.addSso.ssoEnabled"));
        return teamsfx_api_1.err(util_1.sendErrorTelemetryThenReturnError(constants_2.SolutionTelemetryEvent.AddSso, e, telemetryReporter));
    }
...
async function addSso(ctx, inputs, localSettings) {
    ...
    const scaffoldRes = await scaffoldCodeAndResourceTemplate(ctx, inputsNew, localSettings, [typedi_1.Container.get(ResourcePluginContainer_1.ResourcePluginsV2.AadPlugin)], [typedi_1.Container.get(ResourcePluginContainer_1.ResourcePluginsV2.AadPlugin)]);
    if (scaffoldRes.isErr()) {
        ctx.projectSetting.solutionSettings = originalSettings;
