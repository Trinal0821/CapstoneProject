...
async function checkWhetherLocalDebugM365TenantMatches(localDebugTenantId, appStudioTokenProvider) {
    if (localDebugTenantId) {
        const maybeM365TenantId = parseTeamsAppTenantId(await (appStudioTokenProvider === null || appStudioTokenProvider === void 0 ? void 0 : appStudioTokenProvider.getJsonObject()));
        if (maybeM365TenantId.isErr()) {
            return maybeM365TenantId;
        }
        const maybeM365UserAccount = parseUserName(await (appStudioTokenProvider === null || appStudioTokenProvider === void 0 ? void 0 : appStudioTokenProvider.getJsonObject()));
        if (maybeM365UserAccount.isErr()) {
            return maybeM365UserAccount;
        }
        if (maybeM365TenantId.value !== localDebugTenantId) {
            const errorMessage = localizeUtils_1.getLocalizedString("core.localDebug.tenantConfirmNotice", localDebugTenantId, maybeM365UserAccount.value, "state.local.json");
