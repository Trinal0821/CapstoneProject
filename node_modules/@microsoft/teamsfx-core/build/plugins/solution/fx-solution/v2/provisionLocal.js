...
async function provisionLocalResource(ctx, inputs, localSettings, tokenProvider, envInfo) {
    ...
    if (tools_1.isConfigUnifyEnabled()) {
        ...
        if (!localSettings.teamsApp)
            localSettings.teamsApp = {};
        localDebugTenantId = (_a = localSettings.teamsApp) === null || _a === void 0 ? void 0 : _a.tenantId;
    }
    ...
    const pureExistingApp = projectSettingsHelper_1.isPureExistingApp(ctx.projectSetting);
    ...
    if (utils_1.isAzureProject(azureSolutionSettings)) {
        if (plugins.some((plugin) => plugin.name === aadPlugin.name) && aadPlugin.executeUserTask) {
            const result = await aadPlugin.executeUserTask(ctx, inputs, {
                ...
                params: { isLocal: tools_1.isConfigUnifyEnabled() ? false : true },
            }, localSettings, tools_1.isConfigUnifyEnabled()
                ? envInfo
                : { envName: environment_1.environmentManager.getDefaultEnvName(), config: {}, state: {} }, tokenProvider);
            if (result.isErr()) {
                return new teamsfx_api_1.v2.FxPartialSuccess(localSettings, result.error);
            }
        }
    }
    const parseTenantIdresult = utils_1.loadTeamsAppTenantIdForLocal(localSettings, await tokenProvider.appStudioToken.getJsonObject(), envInfo);
