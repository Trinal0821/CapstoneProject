...
async function provisionResource(ctx, inputs, envInfo, tokenProvider) {
    ...
    const tenantIdInTokenRes = await provision_1.getM365TenantId(tokenProvider.appStudioToken);
    ...
    if (tenantIdInConfig && tenantIdInToken && tenantIdInToken !== tenantIdInConfig) {
        return teamsfx_api_1.err(new teamsfx_api_1.UserError("Solution", constants_1.SolutionError.TeamsAppTenantIdNotRight, localizeUtils_1.getLocalizedString("error.M365AccountNotMatch", envInfo.envName)));
    }
    if (!tenantIdInConfig) {
    ...
    if (utils_1.isAzureProject(azureSolutionSettings) && projectSettingsHelper_1.hasAzureResource(ctx.projectSetting, true)) {
        ...
        const consentResult = await provision_1.askForProvisionConsent(ctx, tokenProvider.azureAccountProvider, envInfo);
