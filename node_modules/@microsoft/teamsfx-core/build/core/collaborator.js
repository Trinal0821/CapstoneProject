...
async function listCollaborator(ctx, inputs, envInfo, tokenProvider, telemetryProps) {
    ...
    const result = await collaborationUtil_1.CollaborationUtil.getCurrentUserInfo(tokenProvider.graphTokenProvider);
    if (result.isErr()) {
    ...
    const hasAad = projectSettingsHelper_1.hasAAD(ctx.projectSetting);
    ...
    const appStudioRes = await appStudio.listCollaborator(ctx, inputs, envInfo, tokenProvider.appStudioToken);
    ...
    const aadAppTenantId = (_e = envInfo.state[constants_2.BuiltInFeaturePluginNames.appStudio]) === null || _e === void 0 ? void 0 : _e.tenantId;
    ...
    if (inputs.platform === teamsfx_api_1.Platform.CLI || inputs.platform === teamsfx_api_1.Platform.VSCode) {
        ...
        if (inputs.platform === teamsfx_api_1.Platform.CLI) {
            ...
            ctx.userInteraction.showMessage("info", localizeUtils_1.getLocalizedString("core.collaboration.ListCollaboratorsSuccess", hasAad ? localizeUtils_1.getLocalizedString("core.collaboration.WithAadApp") : ""), false);
...
function getCurrentCollaborationState(envInfo, user) {
    ...
    const aadAppTenantId = (_a = envInfo.state[constants_2.BuiltInFeaturePluginNames.appStudio]) === null || _a === void 0 ? void 0 : _a.tenantId;
...
async function checkPermission(ctx, inputs, envInfo, tokenProvider, telemetryProps) {
    ...
    const appStudio = typedi_1.Container.get(constants_2.BuiltInFeaturePluginNames.appStudio);
    const aadPlugin = typedi_1.Container.get(constants_2.BuiltInFeaturePluginNames.aad);
    const appStudioRes = await appStudio.checkPermission(ctx, inputs, envInfo, tokenProvider.appStudioToken, userInfo);
    if (appStudioRes.isErr()) {
        return teamsfx_api_1.err(appStudioRes.error);
    }
    const permissions = appStudioRes.value;
    const isAadActivated = projectSettingsHelper_1.hasAAD(ctx.projectSetting);
    if (isAadActivated) {
...
async function grantPermission(ctx, inputs, envInfo, tokenProvider, telemetryProps) {
    ...
    try {
        ...
        const userInfo = await collaborationUtil_1.CollaborationUtil.getUserInfo(tokenProvider.graphTokenProvider, email);
        if (!userInfo) {
        ...
        progressBar === null || progressBar === void 0 ? void 0 : progressBar.next(localizeUtils_1.getLocalizedString("core.collaboration.GrantPermissionForUser", email));
        if (inputs.platform === teamsfx_api_1.Platform.CLI) {
            const aadAppTenantId = (_a = envInfo.state[constants_2.BuiltInFeaturePluginNames.appStudio]) === null || _a === void 0 ? void 0 : _a.tenantId;
            const message = [
                {
        ...
        const appStudio = typedi_1.Container.get(constants_2.BuiltInFeaturePluginNames.appStudio);
        const aadPlugin = typedi_1.Container.get(constants_2.BuiltInFeaturePluginNames.aad);
        const appStudioRes = await appStudio.grantPermission(ctx, inputs, envInfo, tokenProvider.appStudioToken, userInfo);
        if (appStudioRes.isErr()) {
            return teamsfx_api_1.err(appStudioRes.error);
        }
        const permissions = appStudioRes.value;
        ...
        if (inputs.platform === teamsfx_api_1.Platform.CLI) {
            ...
            if (projectSettingsHelper_1.hasSPFx(ctx.projectSetting)) {
...
async function getQuestionsForGrantPermission(inputs) {
    ...
    if (isDynamicQuestion) {
        const jsonObject = await globalVars_1.TOOLS.tokenProvider.appStudioToken.getJsonObject();
