...
class EnvironmentManager {
    ...
    async writeEnvState(envData, projectPath, cryptoProvider, envName, isV3) {
        ...
        const data = envData instanceof Map ? tools_1.mapToJson(envData) : envData;
        const secrets = isV3
            ? separateSecretDataV3(data)
            : tools_1.separateSecretData(data);
    ...
    async listRemoteEnvConfigs(projectPath) {
        if (!(await fs_extra_1.default.pathExists(projectPath))) {
            return teamsfx_api_1.err(new error_1.PathNotExistError(projectPath));
        }
        const envConfigsFolder = this.getEnvConfigsFolder(projectPath);
        ...
        if (!(await fs_extra_1.default.pathExists(envConfigPath))) {
            if (envName === this.getLocalEnvName()) {
                ...
                if (projectSettings.isOk()) {
                    const appName = utils_1.getLocalAppName(projectSettings.value.appName);
                    const newEnvConfig = exports.environmentManager.newEnvConfigData(appName);
                    await exports.environmentManager.writeEnvConfig(inputs.projectPath, newEnvConfig, exports.environmentManager.getLocalEnvName());
                }
            }
            if (!(await fs_extra_1.default.pathExists(envConfigPath))) {
    ...
    async loadEnvState(projectPath, envName, cryptoProvider, isV3 = false) {
        ...
        if (!(await fs_extra_1.default.pathExists(envFiles.envState))) {
            ...
            return teamsfx_api_1.ok(new Map([[constants_1.GLOBAL_CONFIG, new teamsfx_api_1.ConfigMap()]]));
        }
        ...
        const data = tools_1.objectToMap(resultJson);
        return teamsfx_api_1.ok(data);
    }
